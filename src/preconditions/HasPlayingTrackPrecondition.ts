...
export class HasPlayingTrackPrecondition extends Precondition {
	public chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		return this.checkPlaying(interaction.guildId);
	}
	...
	public messageRun(message: Message) {
		return this.checkPlaying(message.guildId);
	}

	private checkPlaying(guildId: string | null) {
		if (!guildId) {
			return this.error({
		...
		if (!player.current || !player.playing) {
			return this.error({
				message: "There is no track actively playing.",
			});
		}
